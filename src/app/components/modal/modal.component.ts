  ...
  saveArticle() {
    if (this.form.status === 'VALID') {
      ...
      if (this.form.getRawValue().id === 0) {
        this.addSubscription = this.articleService.addArticle(this.form.getRawValue()).subscribe((response) => {
          console.log(response);
          this.getArticles.emit('');
          this.isModalOpen$.next(false);
        });
      } else {
        this.updateSubscription = this.articleService.updateArticle(this.form.getRawValue()).subscribe((response) => {
          console.log(response);
          this.getArticles.emit('');
          this.isModalOpen$.next(false);
        });
